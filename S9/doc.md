...
Важно: Если тип приказа уже используется в каком-либо документе (`OrderDocument`), его удаление (деактивация) должно быть предотвращено. Рекомендуется возвращать ошибку с кодом 409 (Конфликт) с сообщением, что существуют связанные документы.
...
| signed_by | Кто подписал | Не обязательно | String | max 100 символов |
| is_active | Фильтр по активности | Не обязательно | Boolean | — |
